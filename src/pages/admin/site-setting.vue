<template>
  <z-form
    ...
    :rules="dataRules"
  >
    <template #form-title> 站点信息 </template>
    <template #form-sub-title> 请填写您的站点信息 </template>

    <z-form-item prop="siteName" label="站点名称">
      <el-input id="siteName" v-model="data.siteName" :prefix-icon="Tickets" />
    </z-form-item>

    <z-form-item prop="domain" label="后端站点域名">
      <el-input id="domain" v-model="data.domain" :prefix-icon="Link" />
      <template #tips
        >此地址用于生成直链及本次存储下载使用，请务必保持和服务端地址一样 (需写
        http(s):// 协议头)</template
      >
    </z-form-item>

    <z-form-item prop="frontDomain" label="前端站点域名">
      <el-input v-model="data.frontDomain" :prefix-icon="Link" />
      <template #tips
        >前后端分离后，需配置此地址，会影响 401、403、404 页面的跳转 (需写
        http(s):// 协议头，未前后端分离请保持为空)</template
      >
    </z-form-item>

    <z-form-item prop="avatar" label="头像地址">
      <el-input id="avatar" v-model="data.avatar" :prefix-icon="Avatar" />
      <template #tips
    ...
    <z-form-item label="备案号">
      <el-input id="icp" v-model="data.icp" :prefix-icon="ShieldCheckIcon" />
    </z-form-item>
...
let dataRules = ref({
  ...
  domain: [{ required: true, message: '请输入后端站点域名' }],
  frontDomain: [],
  avatar: [
...
watch(
  ...
  (newVal, oldValue) => {
    ...
    if (oldValue === true) {
      let serverDomain =
        globalConfigStore.zfileConfig.baseUrl || window.location.origin
      ...
      if (serverDomain !== siteDomain) {
        ...
        if (
          globalConfigStore.zfileConfig.baseUrl !== '' &&
