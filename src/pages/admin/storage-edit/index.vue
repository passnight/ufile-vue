<template>
  ...
  >
    ...
    <template #form-sub-title>
      请维护您的存储源信息，可参考
      <a class="link" target="_blank" href="https://docs.zfile.vip/example"
        >ZFile 存储源配置文档</a
      >
    </template>
...
class="inline cursor-pointer ml-2 text-lg relative top-0.5"
              name="copy"
              @click="copyText(item.link + '?clientId=' + encodeURIComponent(storageItem.storageSourceAllParam['clientId'])
                    ) +
                    '&clientSecret=' +
                    encodeURIComponent(
                      storageItem.storageSourceAllParam['clientSecret']
      ...
      },
      {
        validator: (rule, value, callback) => {
          ...
          if (
            ...
            callback(
              new Error(
                '检测到当前 ZFile 站点是 https 协议, 受浏览器限制, 此处也必须是 https 协议, 否则可能无法正常使用.'
              )
            )
            return
          }
