<template>
  <div class="zfile-admin-down-link-body">
    ...
    >
      <template #form-title> 直链设置 </template>
      <template #form-sub-title> 这里可以配置直链相关设置 </template>
      ...
      <z-form-item v-show="data.refererType !== 'no'" label="白名单">
        ...
        <template v-if="data.refererType === 'white_list'" #tips>
          每行输入一个域名，支持 * 通配符，如白名单 *zfile.vip 将只允许
          zfile.vip、www.zfile.vip、demo.zfile.vip 等网站访问。
        </template>
        <template v-if="data.refererType === 'black_list'" #tips>
          每行输入一个域名，支持 * 通配符，如黑名单 *zfile.vip 将禁止所有如
          zfile.vip、www.zfile.vip、demo.zfile.vip 等网站访问。
        </template>
      </z-form-item>
