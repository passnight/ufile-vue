<template>
  <div>
    ...
    <div
      ...
      class="zfile-video-switch-tools"
    >
      <el-button
    ...
    >
      ...
      <div class="zfile-video-tools-item" @click="openTarget('mxplayer-pro')">
        <el-tooltip placement="top">
          <template #content> 使用 MXPlayer(Pro) 播放 </template>
          <img
            src="../../../assets/image/video-mxplayer-pro.png"
            alt="MXPlayer(Pro)"
          />
        </el-tooltip>
      </div>
...
const openTarget = (mode) => {
  switch (mode) {
    case 'download':
      ...
      break
    case 'thunder':
      window.location = `thunder://${btoa(
        'AA' + currentVideo.value.url + 'ZZ'
      )}`
      break
    case 'motrix':
      window.location = `motrix://new-task?uri=${encodeURIComponent(
        currentVideo.value.url
      )}&out=${encodeURIComponent(currentVideo.value.name)}`
      break
    case 'potplayer':
      window.location = `potplayer://${encodeURI(currentVideo.value.url)}`
      break
    case 'iina':
      window.location = `iina://weblink?url=${encodeURIComponent(
        currentVideo.value.url
      )}`
      break
    case 'vlc':
...
const initArtPlayer = async (name, url) => {
  ...
  document.querySelector('.zfile-video-dialog .el-dialog__title').innerHTML =
    name
  if (art) {
    art.destroy()
  }
