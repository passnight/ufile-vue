<template>
  <div class="zfile-header">
    <el-scrollbar>
      <div
        class="zfile-header-breadcrumb box animate__animated animate__fadeIn"
      >
    ...
    <div class="zfile-header-right box animate__animated animate__fadeIn">
      <!-- 功能区 -->
      <div v-if="isNotMobile" class="zfile-header-btn">
        <!-- debug 模式 -->
        <template v-if="storageConfigStore.globalConfig.debugMode">
          <el-tooltip placement="bottom">
            <template #content>
              此功能为 DEBUG 模式下重置管理员密码功能, 使用完后请关闭 DEBUG
              模式并重启服务.
            </template>
            <el-button size="default" type="danger" @click="resetAdminPwd">
              重置管理员密码
            </el-button>
          </el-tooltip>
        </template>
        ...
        <el-tooltip
          ...
          placement="bottom"
        >
          <template #content> 后台管理 </template>
        ...
        >
          <div v-show="route.params.storageKey">
            <el-badge
              :value="uploadProgressInfoStatistics.totalUploadingAndWaiting"
              :hidden="
                uploadProgressInfoStatistics.totalUploadingAndWaiting === 0
              "
              :max="99"
              class="!block"
            >
          ...
          <template #dropdown>
            <el-dropdown-menu class="font-medium">
              <el-dropdown-item
                v-if="storageConfigStore.permission.newFolder"
                @click="newFolder"
              >
        ...
        <div
          ...
          @click="fileDataStore.imgMode = !fileDataStore.imgMode"
        >
          <svg-icon
      ...
      <!-- 存储源选择 -->
      <div v-if="isNotMobile" class="zfile-header-storage-select">
        <el-select
      ...
      <el-dropdown trigger="click" class="top-3">
        ...
        <template #dropdown>
          <el-dropdown-menu>
            ...
            <el-dropdown-item
              v-if="!fileDataStore.imgMode"
              @click="fileDataStore.imgMode = true"
            >
              <svg-icon
                class="text-base mr-2 text-gray-500"
                name="image"
            ...
            <el-dropdown-item
              v-else-if="fileDataStore.imgMode"
              @click="fileDataStore.imgMode = false"
            >
              <svg-icon
                class="text-base mr-2 text-gray-500"
                name="image"
...
const { loadStorageSourceList, currentStorageKey, storageList } =
  useHeaderStorageList()
...
watch(
  () => [
    storageConfigStore.folderConfig.defaultSwitchToImgMode,
    fileDataStore.oldStorageKey,
  ],
  (val, oldValue) => {
...
if (storageConfigStore.globalConfig.customCss) {
  try {
    ...
    console.error(
      '加载自定义 css 加载失败:',
      storageConfigStore.globalConfig.customCss,
      e
    )
  }
}
  ...
  nextTick(() => {
    if (storageConfigStore.globalConfig.customJs) {
      ...
      try {
        ...
        console.log(
          '加载自定义 js 失败: ',
          storageConfigStore.globalConfig.customJs,
          e
        )
      }
    }
